...
function resolveRouteUrl(url: string | URL): URL {
	...
	if (!browserOrigin && typeof url === 'string' && !URL.canParse(url)) {
		throw new TypeError('flamefront route data requires an absolute URL outside the browser.');
	}
...
function staticRouteDataPath(routeUrl: URL, basename: string): string {
	const appPathname = stripBasename(routeUrl.pathname, basename) ?? routeUrl.pathname;
...
function abortable<Data>(pending: Promise<Data>, signal: AbortSignal | undefined): Promise<Data> {
...
function createIsolatedRouteDataClient(
	routing: { readonly basename: string; readonly dataPath: string },
): RouteDataClient {
	...
	const load = <Data = unknown>(
		...
		if (options.reload) cache.delete(key);

		const cached = cache.get(key);
		if (cached) return abortable(cached as Promise<Data>, options.signal);

		const endpoint = source === 'static'
			? new URL(staticRouteDataPath(routeUrl, routing.basename), routeUrl.origin)
			: new URL(routing.dataPath, routeUrl.origin);
		if (source === 'live') endpoint.searchParams.set('url', routeUrl.href);
		...
		const pending = globalThis.fetch(
			...
			.then(async (response) => {
				if (!response.ok) {
					...
					throw new Error(`flamefront ${label} request failed with ${response.status}.`);
				}
...
export function createRouteDataClient(options: RouteDataRoutingOptions = {}): RouteDataClient {
	const routing = normalizeRouting(options);
	if (typeof window === 'undefined') return createIsolatedRouteDataClient(routing);
