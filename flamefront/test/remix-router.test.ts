...
function createRuntime() {
	...
	return {
		createBrowserRouter(routes: TestRoute[], options?: { hydrationData?: unknown }) {
		...
		createStaticHandler(routes: TestRoute[]) {
			return {
				...
				async query(request: Request) {
					const route = routes.find(({ path }) => new URL(request.url).pathname === path);
...
test('creates request-scoped routers and exposes loader hydration state', async () => {
	...
	const first = await adapter.createServerRouter(new Request('https://example.test/items/one'));
	const second = await adapter.createServerRouter(new Request('https://example.test/items/one'));
	...
	assert.equal(adapter.createClientRouter({ hydrationData: first.hydrationData }).kind, 'browser');
});

test('returns redirect Responses from static routing unchanged', async () => {
	const redirect = new Response(null, { status: 302, headers: { Location: '/new' } });
	const adapter = createRemixRouterAdapter(
		[{ path: '/old', result: redirect }],
		createRuntime(),
	);
	const result = await adapter.createServerRouter(new Request('https://example.test/old'));
...
test('retains static route errors in client hydration state', async () => {
	...
	const result = await adapter.createServerRouter(new Request('https://example.test/broken'));
