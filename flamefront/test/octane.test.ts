...
function createTestRenderer(): OctaneRenderer {
	return {
		createStaticRouter: (routes, context) => ({ kind: 'shell', routes, context }),
...
test('passes one document context to the server router and composer', async () => {
	...
	const runtime = createRouteRuntime({
		...
		requestContext: (input) => {
			const context = { token: 'request-scoped', purpose: input.purpose, mode: input.mode };
	...
	let composedParts: { body: string; css: string; hydrationScript: string } | undefined;
	...
	assert.match(composedParts?.hydrationScript ?? '', /flamefront-static-router-hydration/);
