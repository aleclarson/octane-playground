...
test('composes transport concerns behind one basename-aware entry', async () => {
	...
	await writeFile(resolve(clientDirectory, 'static/index.html'), '<main>static artifact</main>');
	...
	try {
		const redirect = await server.fetch(new Request('http://flamefront.test/docs'));
		assert.equal(redirect.status, 302);
		assert.equal(redirect.headers.get('location'), '/docs/client');

		const client = await server.fetch(new Request('http://flamefront.test/docs/client'));
		...
		const serverDocument = await server.fetch(new Request('http://flamefront.test/docs/server'));
		...
		const staticDocument = await server.fetch(new Request('http://flamefront.test/docs/static'));
		assert.equal(await staticDocument.text(), '<main>static artifact</main>');

		const asset = await server.fetch(new Request('http://flamefront.test/docs/assets/app.js'));
